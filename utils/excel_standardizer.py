"""
Excel Standardizer (Canonical Source — utils/)
==============================================
Sprint 16 BUG-110 fix (2026-05-20): Bu modul PFAZ6'dan utils/'e tasindi.

Sebep: PFAZ6 <-> PFAZ12 dairesel import riskini ortadan kaldirmak.
- ESKI yer (PFAZ6 icinde):
    PFAZ6 -> PFAZ12 (lazy, bootstrap_confidence_intervals)
    PFAZ12 -> PFAZ6 (modul-ustu try, ExcelStandardizer)  --> DONGU
- YENI yer (utils/ icinde):
    PFAZ6 -> utils/ (one-way)
    PFAZ12 -> utils/ (one-way)  --> DONGU YOK

Backward compatibility: pfaz_modules/pfaz06_final_reporting/excel_standardizer.py
hala mevcut, sadece bu dosyadan re-export yapar. Eski import yollari kirilmaz.

Tum PFAZ modulleri icin standart Excel formatlama araci.
AdvancedExcelFormatter yerine gecen genel amacli yardimci.

Ozellikler:
- Bold, renkli basliklar (mavi header)
- Hucre uzunluguna gore autosize
- Kosullu bicimlendirme (R² renk skalasi, data bar, trafik isigi)
- Pivot tablo olusturma (openpyxl)
- Iyi/Orta/Kotu renk siniflandirmasi
- Her PFAZ modulunden import edilebilir
...
Tercih edilen kullanim (Sprint 16+):
    ...
    with ExcelStandardizer("rapor.xlsx") as es:
        es.write_sheet("Ozet", df_ozet, conditional_cols=["R2", "RMSE"])
        es.write_pivot("Pivot", df, rows="Target", cols="Model", values="R2")

Geriye uyumlu (eski) kullanim, hala calisir:
